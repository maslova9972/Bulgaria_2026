...
Коды `forum`, `forum-with-breakfast`, `package-*` и `presentation` приходят со страницы форума `/index.html`, коды `breakfast` и `breakfast-forum` — со страницы Business Networking `/breakfast.html`. Код `undecided` доступен на обеих страницах.
...
> **Обязательный шаг перед запуском страницы Business Networking.** Worker создаёт запись с `typecast: false`, поэтому неизвестный вариант single select отклоняется Airtable. В поле `Формат участия` нужно добавить два варианта ровно в таком написании: `Business Networking 6 сентября · бесплатно по приглашению` и `Business Networking 6 сентября и форум 12 сентября`. Если прежние варианты уже существуют, переименуйте их. Разделитель — символ `·` (U+00B7) с пробелами, как в остальных вариантах. Пока варианты не совпадают, заявки с `/breakfast.html` будут возвращать ошибку записи в CRM.
