...
Коды `forum`, `package-*` и `presentation` приходят со страницы форума `/index.html`, коды `breakfast` и `breakfast-forum` — со страницы бизнес-завтрака `/breakfast.html`. Код `undecided` доступен на обеих страницах.
...
> **Обязательный шаг перед запуском страницы бизнес-завтрака.** Worker создаёт запись с `typecast: false`, поэтому неизвестный вариант single select отклоняется Airtable. В поле `Формат участия` нужно добавить два новых варианта ровно в таком написании: `Бизнес-завтрак 6 сентября · 15 €` и `Бизнес-завтрак 6 сентября и форум 12 сентября`. Разделитель — символ `·` (U+00B7) с пробелами, как в остальных вариантах. Пока варианты не добавлены, заявки с `/breakfast.html` будут возвращать ошибку записи в CRM.
